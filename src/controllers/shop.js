...
module.exports = {
  addShop: (req, res) => {
    ...
    shopModel.history_buy().then(result => {
      ...
      productModel.productDetail(data.id_item).then(result => {
        let stock = result[0].stock;
        let Update = result[0].stock - data.qty;
        shopModel.addShop(data, Update).then(result => {
  ...
  finish: (req, res) => {
    ...
    shopModel.history_buy().then(result => {
      ...
      let id_Pembeli = result[0].last_id;
      console.log("yee", id_buy);
      shopModel.finish(id_Pembeli, user).then(result => {
